...
def compute_reward_metrics(batch, config):
    reward_tensor = batch.batch['token_level_scores'].sum(-1)

    reward_metrics = {}

    # import pdb; pdb.set_trace()
    # if 'countdown' in config.data.train_files or 'lighteval' in config.data.train_files or 'DAPO' in config.data.train_files:
    if True:
