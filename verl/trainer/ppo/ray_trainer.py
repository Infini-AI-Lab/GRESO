...
from verl.trainer.ttis.ttis import filter_zero_advantage, select_batch_slice

from verl.trainer.ttis.presampling_selector import DataProfiler
...
class RayPPOTrainer(object):
    ...
    def fit(self):
        ...
        # Set up the generation accumulation
        assert (not (self.config.data.get('generation_accumulation') is not None and hasattr(self.config, 'ttis') and hasattr(self.config.ttis, 'question_scale_size'))), "generation_accumulation and question_scale_size can not be both true."
        ...
        if hasattr(self.config, 'ttis') and hasattr(self.config.ttis, 'question_scale_size'):
            print('question_scale_size is true.')
            generation_accumulation = self.config.ttis.get('question_scale_size')
            print(f'question_scale_size: {generation_accumulation}')
        ...
        for epoch in range(self.start_epoch, self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    # import ipdb; ipdb.set_trace()
                    # TTIS data selection
                    if hasattr(self.config, 'ttis'):
                        orignal_batch = copy.deepcopy(batch)

                        ttis_type = self.config.ttis.get('type', 'pick_all')
                        if ttis_type == 'dynamic_sampling':
                            # import ipdb; ipdb.set_trace()
                            batch = filter_zero_advantage(batch, self.config)
                            if dynamic_sampling_batch is None:
                                dynamic_sampling_batch = batch
                            else:
                                dynamic_sampling_batch = DataProto.concat([dynamic_sampling_batch, batch])

                            if (len(dynamic_sampling_batch) < real_train_batch_size) and (not last_sampling):
                                print('dynamic_sampling_batch size: {}'.format(len(dynamic_sampling_batch)))
                                print('dynamic_sampling_batch size < real_train_batch_size')

                                # calculate adaptive sampling batch size
                                zero_variance_ratio = reward_metrics["examples/non_diverse_examples_ratio"]
                                reminder = real_train_batch_size - len(dynamic_sampling_batch)
                                estimated_sampling_batch_size = int(reminder / (1 - zero_variance_ratio) / self.config.actor_rollout_ref.rollout.n * 1.25)

                                # only for filtering setting
                                if estimated_sampling_batch_size <= default_sampling_batch_size:
                                    last_sampling = True
                                adaptive_sampling_bs = min(estimated_sampling_batch_size, default_sampling_batch_size)
                                adaptive_sampling_bs = max(adaptive_sampling_bs, 64) # minimum batch size = 64

                                if self.config.data.get('full_sampling_bs', False):
                                    adaptive_sampling_bs = default_sampling_batch_size
                                    last_sampling = False

                                print('adaptive_sampling_bs: {}'.format(adaptive_sampling_bs))
                                continue
                            else:
                                print('dynamic_sampling_batch size: {}'.format(len(dynamic_sampling_batch)))
                                print('dynamic_sampling_batch size >= real_train_batch_size or last_sampling')

                                if len(dynamic_sampling_batch) > real_train_batch_size:
                                    batch = select_batch_slice(dynamic_sampling_batch, real_train_batch_size)
                                else:
                                    batch = dynamic_sampling_batch
                                dynamic_sampling_batch = None
                    ...
                    print(f'ref log prob time: {time.time() - start}')
